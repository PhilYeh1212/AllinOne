    ...
    if inTI - fivemin >= 300:
        for sitecode in row:
            try:
                ...
                num = int(sitecode[1])
                sql2 = "%s%s.%s%s" % ('SELECT INVXX11, INVXX10, INVXX08, INVXX13, INVXX12, INVXX20, INVXX19, INVXX18, INVXX05, INVXX15, INVXX14, INVXX01, INVXX02, INVXX03, daq_time, daq_date FROM FSLG_',sitecode[0], 'T2_inv Order by daq_date desc, daq_time desc, INVID asc Limit ', str(sitecode[1]))
                cursor1 = mydb.cursor()
                    ...
                    if inverternum <= 9:
                        ...
                        if INVdata[j][12] <= 0:
                            ...
                            Alarm = '%s#%s%s%s#%s %s#%s#%s%s' % \
                                    (sitecode[0] + '01', sitecode[0] + 'M0', inverternum, '0', INVdata[j][15], INVdata[j][14], 'CLEAR', Alarmcode, '\n')
                            Alarmall = Alarmall + Alarm
                        else:
                            Alarm = '%s#%s%s%s#%s %s#%s#%s%s' % \
                                    (sitecode[0] + '01', sitecode[0] + 'M0', inverternum, '0', INVdata[j][15], INVdata[j][14], 'START', Alarmcode, '\n')
                            Alarmall = Alarmall + Alarm
                    else:
                        raw0 = '%s#%s%s%s#%s %s#%s#%s#%s#%s#%s' % \
                               (
                        ...
                        raw2 = '%s#%s%s%s#%s %s#%s#%s#%s' % \
                               ...
                               INVdata[j][3], INVdata[j][4], '%.2f' % ((INVdata[j][3] * INVdata[j][4]) / 1000) + '\n')
                        RAWline = RAWline + raw2
                        if INVdata[j][12] <= 0:
                            Alarmcode = INVdata[j][12]
                            if Alarmcode <= 0:
