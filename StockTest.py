...
while True:
    ...
    if inTI - LastTI >= 60:
        ...
        df['MA'] = talib.SMA(df['Close'], timeperiod=100)
        df['STD'] = talib.STDDEV(df['Close'], timeperiod=100)
        df['Upper'] = df['MA'] + 2 * df['STD']  #
        df['Lower'] = df['MA'] - 2 * df['STD']  #
        df['macd'] = talib.MACD(df['Close'], fastperiod=12, slowperiod=26, signalperiod=9)[0]
        df['signal'] = talib.MACD(df['Close'], fastperiod=12, slowperiod=26, signalperiod=9)[1]
        df['hist'] = talib.MACD(df['Close'], fastperiod=12, slowperiod=26, signalperiod=9)[2]
        ...
        if Buystock == 0 & Sellstock == 0:
            if (df['K'].tail(1).values > 60 & df['D'].tail(1).values > 60 & df['Close'].tail(1).values > df['MA'].tail(1).values) & \
                    (df['hist'].tail(1).values > 0) & (df['hist'].tail(1).values > df['hist'].tail(2).values):
                now = datetime.now()
        ...
        if Buystock == 1:
            if (df['K'].tail(1).values < 50) & (df['D'].tail(1).values < 50) & (df['Close'].tail(1).values < df['MA'].tail(1).values) & (df['hist'] < 0):
                now = datetime.now()
                current_time = now.strftime("%y%m%d %H:%M:%S")
                print("Current Time =", current_time, "S ")
                Sellstock += 1
